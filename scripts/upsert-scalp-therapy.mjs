#!/usr/bin/env node
// ---------------------------------------------------------------------------
// Surgically upsert the Elv8 Advanced Scalp Therapy 2-Step System (June 2026)
// from the products.json baseline into the live DB catalogue. Only this slug
// is touched -- every other product (including admin-added DB-only rows) is
...
// the row lands identically to an admin "create product" save.
//
// Idempotent: ON CONFLICT upserts, so re-running is safe.
//
// Usage:
//   node scripts/upsert-scalp-therapy.mjs            # dry run (reads only, no writes)
//   node scripts/upsert-scalp-therapy.mjs --commit   # actually write to the DB
// ---------------------------------------------------------------------------
...
// Load .env.local (same parser as upsert-new-pens.mjs)
...
const NEW_SLUGS = ["elv8-advanced-scalp-therapy"];
...
for (const p of toAdd) {
  ...
  console.log(`  [${state}] ${p.slug}  (£${v.price}, ${v.weight})  pos=${nextPos}`);
